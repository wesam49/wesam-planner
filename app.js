...
const STATE_KEY='wesamPlannerV3',AUTO_BACKUP_KEY='wesamPlannerAutoBackup',DATA_SCHEMA_VERSION=7;
...
function customJobPayForMonth(month){
 const events=state.events.filter(e=>e.type==='Zusatzjob'&&e.date.startsWith(month));
 const items=events.map(e=>{
   ...
   return {event:e,name:e.customJobName||'Zusatzjob',hours:h,total:afterDeduction+supplements+Number(e.customFixedBonus||0)};
 });
...
function renderYearOverview(){
 const year=activeFinanceMonth.slice(0,4),rows=[],now=new Date(),currentYear=String(now.getFullYear());
 let totalIncome=0,totalExpense=0,totalSaved=0;
 const monthsForAverage=year<currentYear?12:year===currentYear?now.getMonth()+1:12;
 for(let m=1;m<=12;m++){
 ...
 const divisor=Math.max(1,monthsForAverage);
 document.getElementById('yearSummary').innerHTML=`<div class="row"><div><b>${year}</b><div class="tiny">Jahreswerte</div></div><span class="tag">${monthsForAverage} Monate</span></div><div class="row"><span>Einnahmen gesamt</span><b class="positive">${money(totalIncome)}</b></div><div class="row"><span>Ausgaben gesamt</span><b class="negative">${money(totalExpense)}</b></div><div class="row"><span>Jahresersparnis</span><b class="${totalSaved>=0?'positive':'negative'}">${money(totalSaved)}</b></div><div class="row"><span>Ø Monatliches Einkommen</span><b>${money(totalIncome/divisor)}</b></div><div class="row"><span>Ø Monatliche Ausgaben</span><b>${money(totalExpense/divisor)}</b></div>`;
...
eventType.onchange=toggleCustomJobFields;
function openEvent(id,date){const e=id?state.events.find(x=>x.id===id):null;eventId.value=e?.id||'';eventType.value=e?.type||'VMT';eventDate.value=e?.date||date||iso(new Date());eventStart.value=e?.start||'';eventEnd.value=e?.end||'';paidHours.value=e?.paidHours||'';nightHours.value=e?.nightHours||0;sundayHours.value=e?.sundayHours||0;holidayHours.value=e?.holidayHours||0;eventNote.value=e?.note||'';customJobName.value=e?.customJobName||'';customHourlyRate.value=e?.customHourlyRate||15;customDeductionPct.value=e?.customDeductionPct||0;customNightPct.value=e?.customNightPct||0;customSundayPct.value=e?.customSundayPct||0;customHolidayPct.value=e?.customHolidayPct||0;customFixedBonus.value=e?.customFixedBonus||0;deleteEventBtn.style.display=e?'inline-block':'none';toggleCustomJobFields();eventModal.classList.add('show')}
eventForm.onsubmit=e=>{e.preventDefault();const id=eventId.value||uid(),old=state.events.find(x=>x.id===id),obj={id,type:eventType.value,date:eventDate.value,start:eventStart.value,end:eventEnd.value,paidHours:Number(paidHours.value||0),nightHours:Number(nightHours.value||0),sundayHours:Number(sundayHours.value||0),holidayHours:Number(holidayHours.value||0),note:eventNote.value,workConfirmedAt:old?.workConfirmedAt||null,customJobName:eventType.value==='Zusatzjob'?customJobName.value:'',customHourlyRate:eventType.value==='Zusatzjob'?Number(customHourlyRate.value||0):0,customDeductionPct:eventType.value==='Zusatzjob'?Number(customDeductionPct.value||0):0,customNightPct:eventType.value==='Zusatzjob'?Number(customNightPct.value||0):0,customSundayPct:eventType.value==='Zusatzjob'?Number(customSundayPct.value||0):0,customHolidayPct:eventType.value==='Zusatzjob'?Number(customHolidayPct.value||0):0,customFixedBonus:eventType.value==='Zusatzjob'?Number(customFixedBonus.value||0):0};if(eventModal.dataset.confirmWorkId===id)obj.workConfirmedAt=new Date().toISOString();const i=state.events.findIndex(x=>x.id===id);i>=0?state.events[i]=obj:state.events.push(obj);delete eventModal.dataset.confirmWorkId;eventModal.classList.remove('show');renderAll()};deleteEventBtn.onclick=()=>{state.events=state.events.filter(x=>x.id!==eventId.value);eventModal.classList.remove('show');renderAll()};
