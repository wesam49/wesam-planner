...
function renderYearOverview(){
 ...
 for(let m=1;m<=12;m++){
   ...
   totalIncome+=f.displayIncome;totalExpense+=f.expense;totalSaved+=f.displayRemaining;
   if(f.allRows.length||f.displayIncome>0||f.expense>0)rows.push(`<div class="row" data-finance-month="${mk}"><div><b>${new Intl.DateTimeFormat('de-DE',{month:'long'}).format(new Date(+year,m-1,1))}</b><div class="tiny">Einnahmen ${money(f.displayIncome)} · Ausgaben ${money(f.expense)}</div></div><div class="amount ${f.displayRemaining>0?'positive':f.displayRemaining<0?'negative':''}">${money(f.displayRemaining)}</div></div>`);
 }
 ...
 avgFrom.value=plan.from;avgTo.value=plan.to;targetIncomeAvg.value=plan.incomeTarget||'';targetExpenseAvg.value=plan.expenseTarget||'';targetJobBasis.value=plan.jobBasis||'VMT';targetCustomRate.value=plan.customRate||15;targetCustomRateWrap.style.display=targetJobBasis.value==='custom'?'block':'none';
 const range=monthsBetweenKeys(plan.from,plan.to),n=Math.max(1,range.length);
 const rangeIncome=range.reduce((s,m)=>s+effectiveFinanceSummary(m).displayIncome,0),rangeExpense=range.reduce((s,m)=>s+effectiveFinanceSummary(m).expense,0);
 const avgIncome=rangeIncome/n,avgExpense=rangeExpense/n;
 document.getElementById('yearSummary').innerHTML=`<div class="row"><div><b>${year}</b><div class="tiny">Jahreswerte</div></div><span class="tag">${range.length} Monate im Durchschnitt</span></div><div class="row"><span>Einnahmen gesamt</span><b class="positive">${money(totalIncome)}</b></div><div class="row"><span>Ausgaben gesamt</span><b class="negative">${money(totalExpense)}</b></div><div class="row"><span>Jahresersparnis</span><b class="${totalSaved>=0?'positive':'negative'}">${money(totalSaved)}</b></div><div class="row"><span>Ø Monatliches Einkommen (${plan.from}–${plan.to})</span><b>${money(avgIncome)}</b></div><div class="row"><span>Ø Monatliche Ausgaben (${plan.from}–${plan.to})</span><b>${money(avgExpense)}</b></div>`;
 let status=[];
 if(Number(plan.incomeTarget)>0){
   const target=Number(plan.incomeTarget)*n,diff=rangeIncome-target;
   if(diff>=0)status.push(`<div class="row"><div><b class="positive">✓ Einkommen im Plan</b><div class="tiny">${money(diff)} über deinem Ziel für den gewählten Zeitraum.</div></div></div>`);
   else{
     const missing=-diff,rates={VMT:15,Bib:13.98,Brunner:15},rate=plan.jobBasis==='custom'?Number(plan.customRate||15):rates[plan.jobBasis]||15;
     status.push(`<div class="row"><div><b class="negative">Einkommen unter Ziel</b><div class="tiny">Es fehlen ${money(missing)}. Bei ${money(rate)}/Std. sind das ungefähr ${(missing/Math.max(.01,rate)).toFixed(1)} zusätzliche Arbeitsstunden.</div></div></div>`);
   }
 }
 if(Number(plan.expenseTarget)>0){
   const target=Number(plan.expenseTarget)*n,diff=target-rangeExpense;
   status.push(diff>=0?`<div class="row"><div><b class="positive">✓ Ausgaben im Plan</b><div class="tiny">${money(diff)} unter deinem maximalen Ausgabenziel.</div></div></div>`:`<div class="row"><div><b class="negative">Ausgaben über Ziel</b><div class="tiny">${money(-diff)} über deinem Ziel im gewählten Zeitraum. Dein aktueller Durchschnitt liegt bei ${money(avgExpense)}.</div></div></div>`);
 }
 yearPlanStatus.innerHTML=status.join('')||'<div class="tiny">Trage oben deine Zielwerte ein, um zu sehen, ob Einkommen und Ausgaben auf Kurs sind.</div>';
 ...
 [avgFrom,avgTo,targetIncomeAvg,targetExpenseAvg,targetJobBasis,targetCustomRate].forEach(el=>el.onchange=()=>{
   const p=state.yearPlan[year]||(state.yearPlan[year]={});
   p.from=avgFrom.value||`${year}-01`;p.to=avgTo.value||`${year}-12`;p.incomeTarget=Number(targetIncomeAvg.value||0);p.expenseTarget=Number(targetExpenseAvg.value||0);p.jobBasis=targetJobBasis.value;p.customRate=Number(targetCustomRate.value||15);
   if(p.from>p.to){alert('„Von“ darf nicht nach „Bis“ liegen.');p.to=p.from}
   renderAll();
 });
 targetJobBasis.onchange=()=>{targetCustomRateWrap.style.display=targetJobBasis.value==='custom'?'block':'none';const p=state.yearPlan[year];p.jobBasis=targetJobBasis.value;renderAll()};
}
