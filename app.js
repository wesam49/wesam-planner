...
function renderYearOverview(){
 ...
 if(Number(plan.incomeTarget)>0){
   ...
   if(diff>=0)status.push(`<div class="row"><div><b class="positive">✓ Einkommen im Plan</b><div class="tiny">Ziel: ${money(Number(plan.incomeTarget))}/Monat · Tatsächlich/Geplant: ${money(avgIncome)}/Monat · ${money(diff)} über dem Gesamtziel.</div></div></div>`);
   else{
 ...
 if(Number(plan.expenseTarget)>0){
   ...
   status.push(diff>=0?`<div class="row"><div><b class="positive">✓ Ausgaben im Plan</b><div class="tiny">Limit: ${money(Number(plan.expenseTarget))}/Monat · Tatsächlich/Geplant: ${money(avgExpense)}/Monat · ${money(diff)} unter dem Gesamtlimit.</div></div></div>`:`<div class="row"><div><b class="negative">⚠ Ausgaben über Ziel</b><div class="tiny">Limit: ${money(Number(plan.expenseTarget))}/Monat · Tatsächlich/Geplant: ${money(avgExpense)}/Monat.</div><div class="tiny"><b>${money(-diff)} über dem Gesamtlimit</b> im gewählten Zeitraum.</div></div></div>`);
 }
