(() => {
  // 16.15: income receipt tracking is informational only and never changes finance totals.
  const baseNormalizeState = normalizeState;
  const preloadedIncomeReceipts =
    window.__wesamPreloadState &&
    window.__wesamPreloadState.incomeReceipts &&
    typeof window.__wesamPreloadState.incomeReceipts === 'object'
      ? window.__wesamPreloadState.incomeReceipts
      : null;
  normalizeState = function(input){
    const receipts =
      input && input.incomeReceipts && typeof input.incomeReceipts === 'object'
        ? input.incomeReceipts
        : (preloadedIncomeReceipts || {});
    const out = baseNormalizeState(input);
    out.incomeReceipts = receipts;
    return out;
  };
  state = normalizeState(
    preloadedIncomeReceipts ? {...state, incomeReceipts: preloadedIncomeReceipts} : state
  );
